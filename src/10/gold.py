...
cells_end_trails: dict[tuple[int, int], int] = defaultdict(int)
for top_cell in cells_positions[9]:
    cells_end_trails[top_cell] += 1

# Expandir hacia 0
for i in reversed(range(0, 9)):
    new_cells_end_trails: dict[tuple[int, int], int] = defaultdict(int)
    for cell in cells_positions[i]:
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            if (cx := cell[0] + dx, cy  := cell[1] + dy) in cells_end_trails:
                new_cells_end_trails[cell] += cells_end_trails[(cx, cy)]
    cells_end_trails = new_cells_end_trails

print(sum(trails for trails in cells_end_trails.values()))
